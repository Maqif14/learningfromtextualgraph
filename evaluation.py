...
class Metric(object):
    ...
    def hit_ratio(origin, hits):
        ...
        for user in hits:
            hit_num += hits[user]
        return round(hit_num/total_num,3)
    
    @staticmethod
    def MRR(origin, res):
        """
        Mean Reciprocal Rank (MRR)
        For each user, find the rank of the first relevant item in the prediction list.
        Then average the reciprocal ranks across all users.
        """
        mrr_total = 0
        for user in res:
            predicted_items = [item[0] for item in res[user]]
            relevant_items = set(origin[user].keys())
            reciprocal_rank = 0
        ...
        return round(mrr_total / len(res), 3)
    ...
    def precision(hits, N):
        ...
        return round(prec / (len(hits) * N),3)

    @staticmethod
    def recall(hits, origin):
        recall_list = [hits[user]/len(origin[user]) for user in hits]
        recall = round(sum(recall_list) / len(recall_list),3)
        return recall

    @staticmethod
    def F1(prec, recall):
        if (prec + recall) != 0:
            return round(2 * prec * recall / (prec + recall),3)
        else:
            return 0
    ...
    def RMSE(res):
        ...
        if count==0:
            return error
        return round(math.sqrt(error/count),3)
    ...
    def NDCG(origin,res,N):
        ...
        for user in res:
            ...
            sum_NDCG += DCG / IDCG
        return round(sum_NDCG / len(res),3)
    ...
    def MAP(origin, res, N):
        ...
        return round(sum_prec / len(res), 3)
...
def ranking_evaluation(origin, res, N):
    ...
    for n in N:
        ...
        MRR = Metric.MRR(origin, predicted)
        indicators.append('MRR:' + str(MRR) + '\n')
